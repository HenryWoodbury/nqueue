...
export async function POST(req: NextRequest) {
  try {
    ...
    const playerUniverseArray: PlayerCreateInput[] = parsedData.map(row => ({
    ...
    // Type assertion needed because extended Prisma client types don't expose models in TypeScript
    const result = await (prisma as any)['playerUniverse'].createMany({
